...
def indextocolor(index):    #check if it can convert to image of gtFine
    for label in labels:
        if label.trainId==index:
            color=label.color
            return color
    color=(0,0,0)
    return color
    ...
    for label_dir, index_dir, csv_file in zip([train_dir, val_dir, test_dir], [train_idx_dir, val_idx_dir, test_idx_dir], [train_file, val_file, test_file]):
        ...
        for city in os.listdir(label_dir):
            ...
            for filename in os.listdir(city_dir):
                ...
                img = imageio.imread(img, mode='RGB')   #replace scipy.misc.imread to imageio.imread
                height, weight, _ = img.shape
        
                idx_mat = np.zeros((height, weight))
                for h in range(height):
                    for w in range(weight):
                        color = tuple(img[h, w])    #the RGB of a single image
                        try:
